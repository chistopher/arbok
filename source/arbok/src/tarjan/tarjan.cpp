...
Tarjan::Tarjan(int n, int m, TarjanVariant variant)
    : num_vertices(n), cy(n), co(n), queue_id(n)
{
    inc.reserve(2 * n);
    forest.reserve(2 * n);
    if (variant == TarjanVariant::SET)
        m_impl = make_unique<SetImpl>(n, m);
    if (variant == TarjanVariant::MATRIX)
        m_impl = make_unique<MatrixImpl>(n, m);
    if (variant == TarjanVariant::TREAP)
        m_impl = make_unique<TreapImpl>(n, m);
    if (variant == TarjanVariant::PQ)
        m_impl = make_unique<PQImpl>(n, m);
    if (variant == TarjanVariant::HH)
        m_impl = make_unique<HollowHeapImpl>(n, m);
}
...
long long Tarjan::run(int root)
{
    ...
    for (int i = 0; i < size(q); ++i)
    {
        ...
        int merged = v;
        auto next_in_cyc = [&](int last)
        { return co.find(inc[queue_id[last]].from); };
